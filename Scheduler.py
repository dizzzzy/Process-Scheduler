...
class SchedulerThread(threading.Thread):
    ...
    def swap(self):
        if len(self.active_queue.items) == 0 and len(self.expired_queue.items) != 0:     # swaps queues
            ...
            # Qtemp = Q(len(p_list))
            # highest_prio = 0
            # for i in self.expired_queue.items:
            #     if(self.expired_queue.items[i].process.priority > highest_prio):
            #         Qtemp.enqueue_process(self.expired_queue.items[i])
            #         self.expired_queue.()
        ...
        while True and flag:
            if int((default_timer() - start_time) > 1):  # Start at time 1 second
                for i in range(0, process_length):
                    ...
                    # print process_length
                    # print process_thread_list
                    if process_thread_list[i].process.get_arrival_time() < (default_timer() - start_time) and not process_thread_list[i].process.has_started:
                ...
                while len(self.active_queue.items) != 0:
                    if self.active_queue.getItem(0).process.get_burst_time() > 0:
                        ...
                        if self.active_queue.getItem(0).process.get_timeslot() >= self.active_queue.getItem(0).process.get_burst_time():
                            process_end_time = process_start_time + self.active_queue.getItem(0).process.get_burst_time()
                            going_to_finish = True
                        i = 0
                        while (default_timer() - start_time) < process_end_time:
                            # print "time is" + str((default_timer() - start_time))
                            # print "process end time is" + str(process_end_time)
                                # check if arrival time has passed
                            if process_thread_list[i].process.get_arrival_time() < (default_timer() - start_time) and not process_thread_list[i].process.has_started:
                        ...
                        if going_to_finish:
                            self.active_queue.getItem(0).process.update_burst_time(self.active_queue.getItem(0).process.get_burst_time(), (default_timer() - start_time)) # updates burst time
                            terminated_processes += 1
                            self.active_queue.pop()  # pops the first element in the queue
                            if terminated_processes == process_length:
                                flag = False
                        else:
                            self.active_queue.getItem(0).process.update_burst_time(self.active_queue.getItem(0).process.get_timeslot(), (default_timer() - start_time))  # updates burst time
                            self.expired_queue.enqueue_process(self.active_queue.getItem(0))  # adds the process into expired queue
                            self.active_queue.pop()  # pops the first element in the queue
                        print self.active_queue.size()