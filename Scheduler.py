...
class SchedulerThread(threading.Thread):
    ...
    def swap(self):
        if len(self.active_queue.items) == 0 and len(self.expired_queue.items) != 0:     # swaps queues
            ...
            # highest_prio = 0
            #
            # for i in enumerate(self.expired_queue.items):
            #     if self.expired_queue.items[i[0]].process.priority > highest_prio:
            #         Qtemp.items.append(self.expired_queue.items[i[0]])
            #         self.expired_queue.items.remove(self.expired_queue.items[i[0]])
            #         print "hello"
            # self.expired_queue.items = Qtemp.items
            # Qtemp = Q(len(p_list))
            # self.expired_queue.items[index].process
            ...
            if int((default_timer() - start_time) > 1):  # Start at time 1 second
                ...
                while len(self.active_queue.items) != 0:
                    if self.active_queue.getItem(0).get_burst_time() > 0:
                        ...
                        if going_to_finish:
                            self.active_queue.getItem(0).pause(default_timer() - start_time, going_to_finish)  # pause current process
                            self.active_queue.getItem(0).update_burst_time(self.active_queue.getItem(0).get_burst_time(), (default_timer() - start_time)) # updates burst time
                            ...
                            self.active_queue.getItem(0).pause(default_timer() - start_time, going_to_finish)  # pause current process
                            self.active_queue.getItem(0).update_burst_time(self.active_queue.getItem(0).get_timeslot(), (default_timer() - start_time))  # updates burst time
